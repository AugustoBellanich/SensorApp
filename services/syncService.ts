...
export const syncService = {
  ...
  async pushChanges() {
    ...
    try {
      ...
      if (pendingSensors.length > 0) {
        console.log(
          `📤 [SYNC] Verificando ${pendingSensors.length} sensores pendientes...`,
        );
        for (const sensor of pendingSensors) {
          ...
          if (role !== "owner" && role !== "editor") {
            console.log(
              `🔒 [SYNC] Rol '${role}' en ${cleanId}. Solo marco synced local.`,
            );
          ...
          if (error)
            console.error(`❌ [SYNC] Error sensor ${cleanId}:`, error.message);
          else {
      ...
      console.error("❌ [SYNC] Error sensores:", e);
    }
    ...
    try {
      ...
      if (pendingElectrodes.length > 0) {
        const uniqueSensors = [
          ...new Set(pendingElectrodes.map((e) => e.sensor_id)),
        ];
        ...
        for (const rawSensorId of uniqueSensors) {
          ...
          const electrodesBatch = pendingElectrodes.filter(
            (e) => e.sensor_id === rawSensorId,
          );
          ...
          const { error } = await supabase
            ...
            .upsert(payload);
          if (!error) {
            for (const elec of electrodesBatch)
              await markElectrodeSynced(elec.id);
          } else {
            console.error(
              "❌ [SYNC] Error subiendo electrodos:",
              error.message,
            );
          }
        }
      ...
      console.error("❌ [SYNC] Error electrodos:", e);
    }
    ...
    try {
      ...
      if (pendingReadings.length > 0) {
        const sensorsInBatch = [
          ...new Set(pendingReadings.map((r) => r.sensor_id)),
        ];
        ...
        for (const rawSensorId of sensorsInBatch) {
          ...
          const batch = pendingReadings.filter(
            (r) => r.sensor_id === rawSensorId,
          );
          ...
          if (!error) {
            ...
            await db.runAsync(
              `UPDATE readings_b01 SET is_synced = 1 WHERE id IN (${ids})`,
            );
          } else {
      ...
      console.error("❌ [SYNC] Error lecturas B01:", e);
    }
    ...
    try {
      ...
      if (pendingReadings.length > 0) {
        ...
        for (const rawSensorId of sensorsInBatch) {
          ...
          if (!error) {
            const ids = batch.map((r) => r.id).join(",");
            await db.runAsync(
              `UPDATE readings_c01 SET is_synced = 1 WHERE id IN (${ids})`,
            );
          } else {
      ...
      console.error("❌ [SYNC] Error lecturas C01:", e);
    }
  },
  ...
  async pullChanges() {
    ...
    try {
      ...
      if (permError) {
        console.error("❌ [SYNC] Error bajando permisos:", permError.message);
        return;
      }
      ...
      for (const item of myPermissions) {
        ...
        if (existingLocal && existingLocal.is_synced === 0) {
          console.log(
            `🛡️ [SYNC] Saltando bajada de metadata para ${localSensorId}: Hay cambios locales pendientes.`,
          );
        } else {
  ...
  // ==============================================================================
  // 4. SYNC MANUAL POR RANGO (Local -> Nube, independiente de is_synced)
  // ==============================================================================
  // A diferencia de pushChanges (que solo sube filas con is_synced=0), esta
  // función toma TODO lo que haya en el rango de fechas pedido, sin importar
  // si ya se intentó subir antes. Supabase deduplica por (sensor_id, timestamp)
  // con ON CONFLICT DO NOTHING, así que reenviar filas ya subidas es seguro
  // (no las duplica, solo cuestan ancho de banda de más).
  async syncSensorRange(
    sensorId: string,
    sensorType: "B01" | "C01",
    start: Date,
    end: Date,
    ...
    const cleanId = sensorId.replace("SEN-", "").trim();
    ...
    const rows = await getReadingsInRange(cleanId, sensorType, start, end);
    const received = rows.length;
    if (received === 0) {
      return { received: 0, inserted: 0, duplicates: 0 };
    }

    const table = sensorType === "B01" ? "readings_b01" : "readings_c01";
    ...
    for (let i = 0; i < rows.length; i += CHUNK_SIZE) {
      ...
      if (error) {
        console.error(`❌ [SYNC RANGE] Error subiendo lote:`, error.message);
        const err: any = new Error(`Fallo al subir el lote: ${error.message}`);
        err.partial = { received, inserted, duplicates };
        throw err;
      }

      const insertedInChunk = data?.length || 0;
      inserted += insertedInChunk;
      duplicates += chunk.length - insertedInChunk;

      // Marcamos como sincronizadas las filas de este lote: sea porque recién
      // se insertaron o porque ya existían en la nube, en ambos casos ya
      // confirmamos que el dato está en Supabase.
      const ids = chunk
        .map((r: any) => r.id)
      ...
      if (ids) {
        await db.runAsync(
          `UPDATE ${table} SET is_synced = 1 WHERE id IN (${ids})`,
        );
      }

      processed += chunk.length;
      onProgress?.(processed, received);
    }
