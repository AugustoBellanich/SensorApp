...
export default function SDDataScreen() {
  ...
  // Calcula el intervalo ideal en base al rango real de los datos
  // descargados (acá no hay selector de fechas explícito, se usa el
  // propio rango del CSV bajado por BLE).
  const optimizeChartData = (rawData: any[]) => {
    if (!rawData || rawData.length === 0) {
      return { intervalMs: 3600000, labelFormat: "time" as const };
    }
    const timestamps = rawData.map((d) => d.timestamp);
    const minTs = timestamps.reduce(
      (min, t) => (t < min ? t : min),
      timestamps[0],
    );
    const maxTs = timestamps.reduce(
      (max, t) => (t > max ? t : max),
      timestamps[0],
    );
    return getOptimalInterval(new Date(minTs), new Date(maxTs));
  };
  ...
  // Procesamiento
  const processVisualization = useCallback(
    (rawData: any[]) => {
      ...
      // 2. Pre-cálculo de valores físicos
      const processedRaw = rawData.map((d) => {
        ...
        if (isB01 && unit !== "mV") {
          ...
          if (electrodeConfig[1])
            v1 = calculateMoistureFromSegments(d.e1_mv, electrodeConfig[1]);
          if (electrodeConfig[2])
            v2 = calculateMoistureFromSegments(d.e2_mv, electrodeConfig[2]);
          if (electrodeConfig[3])
            v3 = calculateMoistureFromSegments(d.e3_mv, electrodeConfig[3]);
    ...
    if (data && data.length > 0) {
      ...
      setTimeout(() => {
        try {
          ...
          console.error("Error procesando visualización:", e);
          Alert.alert(
            "Error",
            "No se pudo procesar el gráfico (dataset muy grande o dato inválido).",
          );
        } finally {
          setIsBusy(false);
        }
      ...
      setIsBusy(false);
      if (status !== "error") Alert.alert("Aviso", "No se encontraron datos.");
    }
  };
  ...
  const handleExportExcel = async () => {
    if (downloadedData.length === 0) {
      Alert.alert("Sin datos", "No hay datos.");
      return;
    }
    try {
      setIsBusy(true);
      setBusyMessage("Generando Excel...");
      const isB01 = sensorDb?.type === "B01";
      ...
      const dataToExport = downloadedData.map((item) => {
        ...
        row["Batería (%)"] = Math.round(
          Math.max(0, Math.min(100, ((batMv - 3300) / (4200 - 3300)) * 100)),
        );
        if (isB01) {
          ...
          [1, 2, 3].forEach((idx) => {
            ...
            if (segs && segs.length > 0)
              hv = calculateMoistureFromSegments(mv, segs);
            row[`E${idx} Hv (%)`] = Number(hv.toFixed(2));
            const rho = densities[idx] || 1.3;
            const hg = hv / rho;
      ...
      const uri =
        dir +
        `Sensor_${sensorId}_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  // --- EFECTO: Solo se dispara cuando CAMBIAN LAS UNIDADES ---
  useEffect(() => {
    if (downloadedData.length > 0) {
  ...
  const handleSync = async () => {
    ...
    Alert.alert(
      "Guardar",
      `Se procesarán ${downloadedData.length} registros.`,
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            setIsBusy(true);
            setBusyMessage("Guardando en base de datos...");
            try {
              ...
              if (downloadedData.length > 0) {
                console.log(
                  `   👉 Primer dato (CSV):`,
                  JSON.stringify(downloadedData[0]),
                );
                console.log(
                  `   👉 ID en CSV: '${downloadedData[0].sensor_id}'`,
                );
              }
              ...
              console.log(
                `   👉 ID Final enviado a DB: '${dataToInsert[0].sensor_id}'`,
              );

              let added = 0;
              if (sensorDb?.type === "B01")
                added = await insertReadingsB01(dataToInsert as any);
              else added = await insertReadingsC01(dataToInsert as any);

              console.log(`✅ [SD SAVE] Guardados ${added} registros.`);

              setIsBusy(false);
              Alert.alert("Éxito", `Agregados: ${added}. Nuevos.`, [
                {
                  text: "OK",
                  onPress: () => {
                    resetStatus();
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <View>
          <Text style={styles.title}>Descargar SD</Text>
        </View>
      </View>
          ...
          <View style={styles.filterContainer}>
            {(["Hoy", "1D", "7D", "30D", "Custom"] as TimeRange[]).map((r) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filterBtn,
                  range === r && styles.filterBtnActive,
                ]}
                onPress={() => setRange(r)}
              >
                <Text
                  style={[
                    styles.filterText,
                    range === r && styles.filterTextActive,
                  ]}
                >
                  {r === "Custom" ? "Manual" : r}
                </Text>
          ...
          {range === "Custom" && (
            <View style={styles.customInputRow}>
              ...
              <Text style={styles.helperText}>
                ...
                {new Date(
                  new Date().setDate(
                    new Date().getDate() - (parseInt(customDays) || 0),
                  ),
                ).toLocaleDateString()}{" "}
                hasta Hoy)
              </Text>
                ...
                {[1, 2, 3].map((num) => {
                  ...
                  return (
                    <ExportableChartCard
                      key={num}
                      title={`Electrodo ${num}`}
                      stats={electrodesData[num]?.stats}
