...
export default function GatewayConfigScreen() {
      ...
      try {
        ...
        if (sensor) {
          ...
          setConfig((prev) => ({
            ...
            ...loadedConfig,
            uploaderId: sensorId,
            // Forzamos que si el valor local está vacío, use estrictamente el .env
            supabaseUrl:
              loadedConfig.supabaseUrl && loadedConfig.supabaseUrl.length > 0
                ? loadedConfig.supabaseUrl
                : ENV_SUPABASE_URL,
            anonKey:
              loadedConfig.anonKey && loadedConfig.anonKey.length > 0
                ? loadedConfig.anonKey
                : ENV_SUPABASE_ANON_KEY,
            ingestToken:
              loadedConfig.ingestToken && loadedConfig.ingestToken.length > 0
                ? loadedConfig.ingestToken
                : process.env.EXPO_PUBLIC_INGEST_TOKEN || "",
          }));
        }
      } catch (error) {
  ...
  const setupStatusMonitoring = useCallback(async () => {
    ...
    const readStatusChar = async (charUUID: string) => {
      ...
      const serviceUUID =
        serviceCode === "3" ? BLE_UUIDS.SVC_CONFIG : BLE_UUIDS.SVC_STATUS;
  ...
  const writeConfigToSensor = async (configData: LocalGatewayConfig) => {
    ...
    try {
      const commands = [
        ...
        {
          uuid: BLE_UUIDS.CONFIG.WIFI_SSID,
          val: configData.wifiSsid,
          desc: "WiFi SSID",
        },
        {
          uuid: BLE_UUIDS.CONFIG.WIFI_PASS,
          val: configData.wifiPass,
          desc: "WiFi Password",
        },
        {
          uuid: BLE_UUIDS.CONFIG.UPLOADER_ID,
          val: configData.uploaderId.replace("SEN-", ""),
          desc: "Uploader ID",
        },
        {
          uuid: BLE_UUIDS.CONFIG.SUPABASE_URL,
          val: configData.supabaseUrl,
          desc: "Supabase URL",
        },
        {
          uuid: BLE_UUIDS.CONFIG.ANON_KEY,
          val: configData.anonKey,
          desc: "Anon Key",
        },
        {
          uuid: BLE_UUIDS.CONFIG.INGEST_TOKEN,
          val: configData.ingestToken,
          desc: "Ingest Token",
        },
        {
          uuid: BLE_UUIDS.CONFIG.UPLOAD_INTERVAL,
          val: configData.uploadInterval.toString(),
          ...
          <View style={styles.actionSection}>
            ...
            >
              <MaterialCommunityIcons
                ...
                color="#fff"
              />
              <Text style={styles.actionButtonText}>Forzar Subida Ahora</Text>
            </TouchableOpacity>
            <TimeIntervalInput
              {...({
        ...
        >
          {isWritingConfig ? (
            <View style={{ flexDirection: "row", gap: 10 }}>
              ...
              <Text style={styles.saveButtonText}>ENVIANDO A GATEWAY...</Text>
            </View>
          ) : (
