...
export default function LocalDataScreen() {
      ...
      try {
        ...
        if (results.length === 0) {
          ...
          if (fallbackResults.length > 0) {
            ...
            Alert.alert(
              "Aviso",
              "No se encontraron datos en el rango seleccionado. Mostrando los últimos registros disponibles.",
            );
          }
        }
        ...
        if (results.length > 0) {
          ...
          Alert.alert(
            "Info",
            `No hay datos del ${start.toLocaleDateString()} al ${end.toLocaleDateString()}.`,
          );
        }
      } catch (err) {
  ...
  const handleExportExcel = async () => {
    if (localData.length === 0)
      return Alert.alert("Sin datos", "No hay datos.");
    try {
      setLoading(true);
